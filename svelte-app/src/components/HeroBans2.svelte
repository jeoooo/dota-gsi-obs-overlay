<script>
  ...
  let filename =
    hero_name !== "none" && hero_name !== "banning" ? hero_name : "black_image";
  let filepath = `portraits/${filename}.png`;
</script>

<div class="relative w-full h-full bg-red-600 animate-fade-in">
  <img
    class="h-[55px] w-full grayscale"
    src={filepath ? filepath : "portraits/black_image.png"}
    alt=""
  />
  <div class="absolute inset-0 bg-red-600 opacity-60"></div>

  {#if hero_name == "banning"}
    <img
